  ...
  def _handle_PacketIn (self, event):
    ...
    if oldloc is None:
      if packet.src.is_multicast == False:
        ...
        print "Learned %s at %s %i" %(packet.src, loc[0], loc[1])
        
        if loc[0]=='00-00-00-00-00-02' and loc[1]=='2':
          drop()
          print packet
        # by zxch
        log.debug("New host")
   
        #core.l2_multi.raiseEvent(HostNew, packet.src, loc[0], loc[1], self.connection)
        # end zxch
      ...
      if core.openflow_discovery.is_edge_port(loc[0].dpid, loc[1]):
        ...
        if packet.src.is_multicast == False:
          ...
          # by zxch
          log.debug("host another port")
          core.l2_multi.raiseEvent(HostChange, packet.src, loc[0], loc[1], self.connection)
          # end zxch
...
class HostHandle (object):
  ...
  def _handle_HostNew(self, event):
    print "Port %s on Switch %s has come up a new host" %(event.port,event.switch)
    print event.hostmac
