...
global SERVER_SW_PORT
SERVER_CONTROLLER_PORT = 5560
OVS_IP = "192.168.1.1"
OVS_MAC = "2A:EF:FE:9A:43:E2"
SW_IP = "10.0.0.200"
SW_PORT = 5561
RTP_PORT = 5004
# end zxch
...
# *****************************************************by zlk*******************************************************
  ...
  def _install (self, switch, in_port, out_port, match, buf = None):
    ...
    msg.match = match
    #print match
    if(msg.match.dl_type == 0x0800 and msg.match.nw_proto == 17 ): #and msg.match.tp_dst==5004
           print 'path install'
...
# 下发缓存视频流表
        if isinstance(packet.next, ipv4) and packet.next.srcip == SERVER_IP:
          if isinstance(packet.next.next, udp) and packet.next.next.dstport == RTP_PORT:
...
#************************************************** by zlk ********************************************
  ...
  def _handle_ClientOld (self, event):
    ...
    s.send('rtpplay'+'->'+str(event.clientip))
    print 'rtpplay'+'->'+str(event.clientip)
    log.debug("rtpplay sent")
    s.close()
  
  def _handle_RtpVideo (self,event):
    print "rtp video stream has come"
    print 'out_port:'
    print mac_map[EthAddr(clientmac)][1] 
    ...
    msg.actions.append(of.ofp_action_output(port = mac_map[EthAddr(clientmac)][1]))
    ...
    #event.connection.send(msg)
    core.openflow.connections[2].send(msg)
    print 'messege sent'
...
def fromDeviceManager():
    ...
    while True:
	    ...
	    serverip=inf[4]
            ...
            if num=='1':
               core.l2_multi.raiseEvent(ClientNew,clientip) 
               ...
               core.l2_multi.raiseEvent(ClientOld,clientip)
    s.close()
