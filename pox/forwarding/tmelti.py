...
mesh_bat0_mac = {}
channel = {}  ##dpid : {dpid : linksituation} the larger the value of linksituation is,the worse the linksituation is 
#init_channel = {}
temp_channel = {}
channel_flag = 0
defaultchannel = 100
...
def handle(client_sock, client_addr):
  ...
  if len(cha_socinited) == sum(cha_socinited.values()):   # if all the channels have been set to the real value 
    ...
    for i in cha_reverse:
      ...
      if cha_reverse[i].count(100) == len(cha_reverse[i]):
        ...
        for m in temp:
          for n in temp[m]:
            path = paths_map[m].pop(n)    # delete the relating existed paths
  ...
  file.write('***************************************************************************\n')
  file.write('clients:'+str(mac_map)+'\n')
  file.write('***************************************************************************\n')
