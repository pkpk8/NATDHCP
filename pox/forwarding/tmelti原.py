...
def _get_path (srcmac, dstmac ):
  ...
  if srcdpid in paths_map:
    if dstdpid in paths_map[srcdpid]:
      if path != paths_map[srcdpid][dstdpid]:      # the path is different with the path stored in paths_map 
        print "***************************************"
        print "Path alternated!!!!!"
        print paths_map[srcdpid][dstdpid]
        print " --> "
        print path
        print pathre
        print "***************************************"
        ...
        file.write(str(paths_map[srcdpid][dstdpid]))
        file.write(" --> ")
        file.write(str(path))
        file.write(str(pathre))
        file.write("***************************************\n")
        paths_map[srcdpid][dstdpid] = path
        paths_map[dstdpid][srcdpid] = pathre
    else:
      print "**************************************"
      print "New path get!!!!!"
      print path
      ...
      file.write("***************************************\n")
      paths_map[srcdpid][dstdpid] = path
      if not paths_map.has_key(dstdpid):
        paths_map[dstdpid] = {}
      paths_map[dstdpid][srcdpid] = pathre
  else:	
    print "***************************************"
